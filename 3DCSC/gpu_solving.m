...
%% Parameters extractions
	...
	gR      = gpuArray(R0);
	gMf     = gpuArray(M0);
%     gM      = gpuArray(ifft2(gMf,'symmetric')); change 
...
%     gS      = gpuArray(ifft2(gMf,'symmetric')); change
...
%         gSl  = ifft2(gSlf,'symmetric'); change
%         gSh  = ifft2(gShf,'symmetric'); change
        gSl  = ifft2(gSlf);
        gSh  = ifft2(gShf);
...
%         gS = ifft2(gSf2, 'symmetric'); change
        gS = ifft2(gSf2);
        ...
        if opt.isDisplay 
            % show dictionary
%             G1 = gather(PzpT1(gG(:,:,:,1:numDicts)));
%             G2 = gather(PzpT2(gG(:,:,:,numDicts+1:2*numDicts)));
%             G3 = gather(PzpT3(gG(:,:,:,2*numDicts +1:size(gG,4))));
%             
%             tmp1 = squeeze(G1(:,:,1,:));
%             tmp2 = squeeze(G2(:,:,1,:));
%             tmp3 = squeeze(G3(:,:,1,:));
...
%             
%             figure(21);
%             imdisp(tiledict(tmp1)); drawnow; 
%             figure(22);
%             imdisp(tiledict(tmp2)); drawnow;
%             figure(23);
%             imdisp(tiledict(tmp3)); drawnow;
...
%             subplot(1,3,2);
%             imshow((gather(gSl(:,:,1))),[0 1]);
%             title('Low');
%             
%             subplot(1,3,3);
%             imshow((gather(gSh(:,:,1))),[0 1]);
%             title('gSh');
%             drawnow();
...
% 
%             subplot(1,3,2);
%             imshow(imag(gather(gS0(:,:,1))),[0 1]);
%             title('Fullsample');
%             
%             subplot(1,3,3);
%             imshow(imag(gather(gS(:,:,1))),[0 1]);
%             title('Reconstruction');
%             drawnow();
            
            figure(6);
            imshow(scale_img(abs([angle(gather(gM(:,:,l))); angle(gather(gS0(:,:,l))); angle(gather(gS(:,:,l)))])));
            colormap jet;
            title('concat');
            drawnow();
